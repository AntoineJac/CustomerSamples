...
VpaidVideoPlayer.prototype.initAd = function(
  ...
  this.skipMessage = data.skipMessage;
  this.positionMessage = data.positionMessage;
...
(positionMessage == undefined) ? (closeButton.style.bottom="20px") : (closeButton.style.top="20px");
...
VpaidVideoPlayer.prototype.countDownTimer2 = function(x, y) {
(x == undefined) ? skipMessage = "Skip Ad in " : message = x;
(y == undefined) ? waitMessage = "Click To Skip Ad" : message = y;
...
if (CountDownTime < 0) {
        ...
        closeButton.innerHTML = waitMessage;
  ...
  document.getElementById("demo").innerHTML = skipMessage + seconds + " s ";
}
   CountDownTime-- 
}, 1000);
};
...
(sizeD == 15)? (w = 300, h = 250) : w = (w = 728, h = 90);
   if (response.status == "ok") {
       ...
       for (var i = 0; i < response.ads.length; i++) {
           ...
           if (ad.status == "ok") {
               if (ad.type == "script") {
                   document.write("<div id = 'test' style = 'width: "+w+"px; height: "+h+"px; top: 10%; margin: 0 auto; position: relative;'><script type='text/javascript'>"+ad.script+"</scr"+"ipt></div>"); 
                   createButton();
                   countDownTimer();
               }
               if (ad.type == "html") {
                   document.write("<div id = 'test' style = 'width: "+w+"px; height: "+h+"px; top: 10%; margin: 0 auto; position: relative;'>"+ad.html+"</div>"); 
                   createButton();
                   countDownTimer();
               }
           } else {
...
window.createButton = this.createCloseButton;
window.countDownTimer = this.countDownTimer2(this.skipMessage, this.waitMessage);
window.sizeD = this.sizeId;
window.positionMessage = this.positionMessage;


   setTimeout(function(){
    if (window.rubiconTimeout) {
  stopAd(); }
},this.timeoutRubicon);
